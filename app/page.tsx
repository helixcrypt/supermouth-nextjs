...
const pressLogos = [
  ...
  'Health Magazine', 'Bustle', 'Business Insider', 'BuzzFeed', 'Fox News',
  'Refinery29', 'Cosmopolitan', 'Seventeen', 'PopSugar',
]

const ageCategories = [
  { label: 'Kids', href: '/collections/shop-all-kids', img: 'https://pics.io/public/images/6a161386906ce9ec00be32d3/cdn', desc: 'Complete care for growing smiles — safe, fun products from first teeth to preteens.' },
  { label: 'Teens & Adults', href: '/collections/ages-13-and-up', img: 'https://pics.io/public/images/6a1613855e27efbdf961ff04/cdn', desc: 'Complete daily care for confident smiles — advanced products for every routine.' },
  { label: 'Orthodontics', href: '/collections/ortho', img: 'https://pics.io/public/images/6a1613851c5e702c945fbf64/cdn', desc: 'Complete ortho care for braces, aligners, and retainers — protect smiles in transition.' },
  { label: 'Pregnancy', href: '/collections/pregnancy', img: 'https://pics.io/public/images/6a1613851c5e702c945fbf65/cdn', desc: 'Safe, complete oral care through pregnancy and nursing — support for you and baby.' },
]
...
export default function HomePage() {
  return (
    <>
      ...
      <section className="relative bg-sm-navy overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <div className="text-center max-w-4xl mx-auto animate-fade-in-up">
            <p className="text-sm-accent text-sm font-semibold tracking-widest uppercase mb-4">Founded by Dentists · 125,000+ Families</p>
            ...
            <div className="flex items-center justify-center gap-6 text-sm text-sm-gray mb-6">
              <span>★★★★★ <strong className="text-white">4.8/5</strong></span>
              <span>10,000+ verified reviews</span>
              <span>500,000+ orders shipped</span>
            </div>
            <p className="text-sm-gray text-sm mb-8">30-Day Money-Back Guarantee · Love it or full refund</p>

            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-10 text-sm text-left max-w-2xl mx-auto">
              <p className="text-sm-white/80 leading-relaxed">
                The toothpaste in your bathroom may contain lead. 90% of tested brands do. 65% contain arsenic.†
                SuperMouth independently third-party tests every batch for heavy metals — so you never have to wonder.
              </p>
              <p className="text-sm-gray/60 text-xs mt-2">† Source: Lead Safe Mama, LLC independent testing (2025) of 51 toothpaste products, reported by The Guardian and Fortune.</p>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-6">
              {['Kids', 'Adults', 'Orthodontics', 'Pregnancy'].map(cat => (
                <Link key={cat} href={`/collections/${cat.toLowerCase()}`}
                  className="text-sm text-sm-accent hover:underline border border-sm-accent/30 px-4 py-1.5 rounded-full text-xs font-semibold hover:bg-sm-accent/10 transition-colors">
                  {cat}
                </Link>
              ))}
            </div>
      ...
      {/* PRESS TICKER */}
      <div className="overflow-hidden py-6 border-b border-white/10 bg-white/5">
        <div className="ticker-track flex gap-12 whitespace-nowrap" style={{ width: 'max-content' }}>
          {[...pressLogos, ...pressLogos].map((logo, i) => (
            <span key={i} className="text-sm-gray/60 text-sm font-semibold">{logo}</span>
          ))}
        </div>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <h2 className="font-display font-black text-3xl md:text-4xl text-center mb-12">How to Get Started</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { img: 'https://cdn.supermouth.com/website/SM-website_get_started_1_guide_you.jpg', title: 'Let Us Guide You', desc: 'Take a quick quiz and get a custom plan from our experts — tailored to your age, stage, and needs.', cta: 'Take Assessment', href: '/assessment' },
            { img: 'https://cdn.supermouth.com/website/SM-website_get_started_2_shop_systems.jpg', title: 'Shop Systems', desc: 'Expert-curated systems for every age and stage, offering best-in-class options for all budgets.', cta: 'Shop Systems', href: '/systems-overview' },
            { img: 'https://cdn.supermouth.com/website/SM-website_get_started_3_shop_products.jpg', title: 'Shop Products', desc: 'Discover nearly 200 groundbreaking products designed for all ages, each at the pinnacle of innovation.', cta: 'Shop Products', href: '/shop-all' },
          ].map(item => (
            <div key={item.title} className="bg-white/5 rounded-2xl overflow-hidden group">
              ...
              <div className="p-6">
                ...
                <p className="text-sm text-sm-gray mb-4">{item.desc}</p>
      ...
      <section className="bg-white/5 section-divider py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
            {systems.map(sys => (
              <Link key={sys.label} href={sys.href} className="group bg-sm-dark rounded-2xl overflow-hidden hover:ring-1 hover:ring-sm-accent/50 transition-all">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
          {productCategories.map(cat => (
            <Link key={cat.label} href={cat.href} className="group bg-white/5 rounded-2xl overflow-hidden hover:bg-white/8 transition-colors">
                ...
                <ul className="space-y-1">
                  {cat.bullets.slice(0, 3).map(b => (
                    <li key={b} className="text-xs text-sm-gray flex gap-1.5">
                      <span className="text-sm-accent mt-0.5">✓</span>{b}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-5">
            {superDifference.map((item, i) => (
              <div key={item.letter} className={`flex gap-6 items-center ${i % 2 === 1 ? 'flex-row-reverse' : ''} bg-sm-dark rounded-2xl p-6 group`}>
