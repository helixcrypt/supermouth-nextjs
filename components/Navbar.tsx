...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      {/* Nav bar — position:relative so the shop mega menu can use absolute positioning within it */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" onClick={closeAll} className="flex-shrink-0">
              <Image src="/cdn/supermouth-logo.png" alt="SuperMouth" width={160} height={40} className="h-9 w-auto" />
            </Link>

            <nav className="hidden md:flex items-center gap-6">

              {/* SHOP — mega menu anchored to the nav container, not the button */}
              <div className="relative" onMouseEnter={() => setActiveDropdown('shop')} onMouseLeave={() => setActiveDropdown(null)}>
                ...
                {activeDropdown === 'shop' && (
                  <div
                    className="fixed bg-white text-sm-navy shadow-2xl border border-gray-100 rounded-2xl z-50 p-6"
                    style={{ top: 'auto', left: '50%', transform: 'translateX(-50%)', width: '860px', marginTop: '0' }}
                    onMouseEnter={() => setActiveDropdown('shop')}
                    onMouseLeave={() => setActiveDropdown(null)}
                  >
                    <div className="grid grid-cols-4 gap-6">
                      {/* Col 1 — Shop by Person */}
                      <div>
                        ...
                        <ul className="space-y-2">
                          {shopByPerson.map(item => (
                            <li key={item.label}>
                              <Link href={item.href} onClick={closeAll} className={`text-sm hover:text-sm-teal transition-colors flex items-baseline justify-between gap-2 ${item.teal ? 'font-bold text-sm-teal' : 'text-sm-navy'}`}>
                      ...
                      <div>
                        ...
                        <ul className="space-y-1.5 mb-3">
                          {[
                            ...
                            <li key={item.label}>
                              <Link href={item.href} onClick={closeAll} className={`text-sm hover:text-sm-teal transition-colors ${item.highlight ? 'bg-yellow-50 px-2 py-1 rounded-lg block' : 'text-sm-navy'}`}>{item.label}</Link>
                            </li>
                          ))}
                        </ul>
                        ...
                        <ul className="space-y-1.5 mb-3">
                          {[
                            ...
                            <li key={item.label}>
                              <Link href={item.href} onClick={closeAll} className={`text-sm hover:text-sm-teal transition-colors ${item.teal ? 'font-bold text-sm-teal' : 'text-sm-navy'}`}>{item.label}{item.teal ? ' ->' : ''}</Link>
                            </li>
                          ))}
                        </ul>
                        ...
                        <ul className="space-y-1.5">
                          {[
                            ...
                            <li key={item.label}>
                              <Link href={item.href} onClick={closeAll} className={`text-sm hover:text-sm-teal transition-colors flex items-center gap-2 ${item.highlight ? 'bg-yellow-50 px-2 py-1 rounded-lg' : 'text-sm-navy'}`}>
                      ...
                      <div>
                        ...
                        <ul className="space-y-1.5 mb-4">
                          {systemLinks.map(item => (
                            <li key={item.label}>
                              <Link href={item.href} onClick={closeAll} className="text-sm text-sm-navy hover:text-sm-teal transition-colors flex items-baseline justify-between gap-2">
                                <span>{item.label}</span>
                                {item.tag && <span className="text-xs text-sm-gray flex-shrink-0">{item.tag}</span>}
                              </Link>
                            </li>
                  ...
                  <div className="absolute top-full left-0 bg-white border border-gray-100 rounded-2xl shadow-xl z-50 w-[480px]">
                    <div className="grid grid-cols-2">
                      <div className="py-4 px-5 border-r border-gray-100">
                        ...
                        {learnLinks.map(l => (
                          <Link key={l.href} href={l.href} onClick={closeAll} className="block py-2.5 text-sm text-sm-navy hover:text-sm-teal transition-colors font-medium border-b border-gray-50 last:border-0">{l.label}</Link>
                        ))}
                      </div>
                      <div className="relative overflow-hidden rounded-r-2xl min-h-[200px]">
                        <Link href="/learn" onClick={closeAll}>
                          <Image src="/cdn/blogs-hero.png" alt="Explore the science behind superior oral care" width={400} height={300} className="w-full h-full object-cover absolute inset-0" />
              ...
              {/* WHY SUPERMOUTH */}
              <div className="relative" onMouseEnter={() => setActiveDropdown('why')} onMouseLeave={() => setActiveDropdown(null)}>
                    ...
                    <div className="grid grid-cols-2">
                      ...
                      <div className="relative overflow-hidden rounded-r-2xl min-h-[200px]">
                        <Link href="/what-makes-us-super" onClick={closeAll}>
                          <Image src="/cdn/why-supermouth-header.jpg" alt="Why SuperMouth" width={400} height={400} className="w-full h-full object-cover absolute inset-0" />
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
                          <div className="absolute bottom-0 p-4">
                            <p className="text-white font-display font-bold text-sm leading-snug mb-1">Revolutionizing oral care with safe, effective, and fun products</p>
                            <p className="text-white/70 text-xs mb-2">Founded by dentists who could not find the right products so they invented them!</p>
              ...
              <Link href="https://supermouthpro.com" target="_blank" onClick={closeAll} className="text-sm font-semibold italic hover:text-sm-yellow transition-colors">
                For Dental Pros
              </Link>
            </nav>

            <div className="flex items-center gap-3">
              <Link href="/assessment" onClick={closeAll} className="hidden md:block bg-sm-yellow text-sm-navy font-bold px-4 py-2 rounded-full text-xs hover:opacity-90 transition-opacity whitespace-nowrap">
                TAKE ASSESSMENT
              </Link>
              <Link href="/account" onClick={closeAll} className="hidden md:block text-sm font-semibold border border-white/40 px-3 py-1.5 rounded-full text-xs hover:bg-white/10 transition-colors">
                SIGN IN
              </Link>
              <button onClick={openCart} className="relative p-2 hover:text-sm-yellow transition-colors" aria-label="Open cart">
                ...
                {itemCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-sm-yellow text-sm-navy text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{itemCount}</span>
                )}
              </button>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-sm-navy text-white border-t border-white/10 px-4 pb-6 overflow-y-auto max-h-[85vh]">
          <div className="pt-4 space-y-1">
            ...
            {[{ label: 'Shop All', href: '/shop-all' }, ...shopByPerson].map(l => <Link key={l.href + l.label} href={l.href} onClick={closeAll} className="block py-2 text-sm hover:text-sm-yellow">{l.label}</Link>)}
            <p className="text-xs text-white/50 font-bold uppercase tracking-widest mt-4 mb-2">Shop Products</p>
            {[{ label: 'Shop All Products', href: '/shop-all' }, ...shopProducts].map(l => <Link key={l.href + l.label} href={l.href} onClick={closeAll} className="block py-2 text-sm hover:text-sm-yellow">{l.label}</Link>)}
            <p className="text-xs text-white/50 font-bold uppercase tracking-widest mt-4 mb-2">Systems & Bundles</p>
            {[{ label: 'All Systems - Save 15%', href: '/systems-overview' }, { label: 'All Bundles', href: '/supermouth-bundles' }].map(l => <Link key={l.href} href={l.href} onClick={closeAll} className="block py-2 text-sm hover:text-sm-yellow">{l.label}</Link>)}
            ...
            <div className="pt-4">
              <Link href="/assessment" onClick={closeAll} className="block w-full text-center bg-sm-yellow text-sm-navy font-bold py-3 rounded-full text-sm">TAKE ASSESSMENT</Link>
            </div>
          </div>
        </div>
      )}
